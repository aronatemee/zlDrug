...
, zz.病人id || '/' || zz.日期 || '/' || zz.匹配 || '/' || zz.嘱托换页 || '/' || zz.咀嚼泡腾换页 ||  '/' || zz.护士实际一次发送哪几天 ||  '/' || trunc(zz.该药袋第几种药 / 6) as 换页标识, zz.药品id
, zz.匹配 from (
--计算原本药袋有多少种药。
...
, ww.日期, ww.匹配, row_number() over(partition by ww.床号0, ww.病人id, ww.日期, case when ww.医生嘱托 is not null then ww.药品id || ww.医生嘱托 else '' end, ww.匹配
 order by ww.床号0 asc, ww.病人id asc, ww.日期 asc, case when ww.医生嘱托 is not null then ww.药品id || ww.医生嘱托 else '' end, ww.匹配 desc, ww.药品id asc) as 该药袋第几种药, ww.护士实际一次发送哪几天 
 from (
--★★★长嘱必定打印药袋★★★
select * from (
select 
...
select distinct g.规格, a.库房id, a.药品id, a.医嘱id, e.出院病床 as 床号, e.病人id, e.主页id, NVL(h.名称, g.名称) as 医嘱
  , RTRIM(to_char(b.单次用量/d.剂量系数,'fm990.99999'), '.')||d.住院单位 as 用量,a.用法
  , datesplit.要求日期 as 日期, b.执行频次, b.频率次数, b.首日数次, f.名称 as 科室, b.姓名||decode(b.婴儿,0,'','【婴】') as 姓名, a.年龄, e.住院号, b.医生嘱托, b.开始执行时间
  , datesplit.要求日期 as 护士实际一次发送哪几天
  from 药品收发记录 a,病人医嘱记录 b ,药品规格 d,病案主页 e,部门表 f,病人医嘱发送 c,病人医嘱发送 c2, 收费项目目录 g, 收费项目别名 h
...
order by temp_origin_continuous.床号 asc, temp_origin_continuous.病人id asc, temp_origin_continuous.护士实际一次发送哪几天 asc, drugbag_rec.匹配 desc, temp_origin_continuous.药品id
...
select 
  ...
  , temp_origin_temporary.开始执行时间, RPAD('服药日期: ' || temp_origin_temporary.日期, 30, ' ') as 护士实际一次发送哪几天
from (
--源数据集
select distinct g.规格, a.库房id, a.药品id, a.医嘱id, e.出院病床 as 床号, e.病人id, e.主页id, NVL(h.名称, g.名称) as 医嘱
  , RTRIM(to_char(b.单次用量/d.剂量系数,'fm990.99999'), '.')||d.住院单位 as 用量,a.用法
  , to_char(c.首次时间, 'yyyy-mm-dd') as 日期, b.执行频次, b.频率次数, b.首日数次, f.名称 as 科室, b.姓名||decode(b.婴儿,0,'','【婴】') as 姓名, a.年龄, e.住院号, b.医生嘱托, b.开始执行时间
  from 药品收发记录 a,病人医嘱记录 b ,药品规格 d,病案主页 e,部门表 f,病人医嘱发送 c, 收费项目目录 g, 收费项目别名 h, 医嘱执行时间 j
  , (
...
order by ww.床号0 asc, ww.病人id asc, ww.护士实际一次发送哪几天 asc, case when ww.医生嘱托 is not null then ww.药品id || ww.医生嘱托 else '' end, ww.匹配 desc, ww.药品id asc
) zz
order by zz.床号0 asc, zz.病人id asc, zz.护士实际一次发送哪几天 asc, zz.嘱托换页, zz.咀嚼泡腾换页, zz.匹配 desc, zz.药品id asc
) xxx
